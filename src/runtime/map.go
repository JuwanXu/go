...
/**
 * map.go文件包含Go的 map 类型的实现。
 *
 * map 只是一个哈希表。数据被安排在一系列存储桶中。每个存储桶最多包含8个键/元素对。
...
const (
	...
	// 可能的 tophash 值。我们为特殊标记保留一些可能性。
	// 每个存储桶（包括其溢出存储桶，如果有的话）在evacuated*状态下将具有全部或没有条目
	//（除了evacuate()方法期间，该方法仅在 map 写入期间发生，因此在此期间没有其他人可以观察该 map）。
	...
	hashWriting  = 4 // a goroutine is writing to the map // 一个goroutine正在写 map
	sameSizeGrow = 8 // the current map growth is to a new map of the same size // 新 bucekt 与 old_bucket size相同
...
type mapextra struct {
	...
	// 如果key和elem都不包含指针并且是内联的，则我们将存储桶类型标记为不包含指针。这样可以避免扫描此类 map。
...
type bmap struct {
	...
	// 但是它使我们可以省去填充，例如，map[int64] int8。后跟一个溢出指针。
}
...
/**
 * 当在编译时已知hint最多为bucketCnt并且需要在堆上分配 nao 时，
 * makemap_small实现了make(map[k]v)和make(map[k]v, hint)的Go map 创建。
 * @param
 * @return
 **/
 ...
 * 如果编译器确定可以在堆栈上创建 map 或第一个存储桶，则h和/或存储桶可能为非nil。
...
/**
 * 查询 map 的方法        (a := map[key] 方式)。从不返回nil，如果键不在 map 中，它将返回对elem类型的零对象的引用。
 * 注意：返回的指针可能会使整个 map 保持活动状态，因此请不要持有很长时间。
 * @param
 * @return
 **/
...
/**
 * 与mapaccess类似，但是如果map中不存在key，则为该 key 分配一个插槽。
...
func mapiterinit(t *maptype, h *hmap, it *hiter) {
	...
	if unsafe.Sizeof(hiter{})/goarch.PtrSize != 12 { // hiter{}的大小必须是指针大小的12倍
